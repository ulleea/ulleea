...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # Your code goes here. ################################################
        np.divide(np.exp(self.output), np.sum(np.exp(self.output), axis=1, keepdims=True), out = self.output)
        return self.output
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # Your code goes here. ################################################
        self.output = np.log(np.divide(np.exp(self.output), np.sum(np.exp(self.output), axis=1, keepdims=True)))
        #print(self.output)
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(gradOutput.shape[0]):
            local_derivative[i] = np.eye(gradOutput.shape[1])
            o = np.ones(gradOutput.shape[1]).reshape(-1, 1)
            d = np.divide(np.exp(input[i]), np.sum(np.exp(input[i]))).reshape(1, -1)
            t = np.dot(o,d)
            local_derivative[i]-= t
        self.gradInput = np.matmul(gradOutput[:,None,:], (local_derivative))[:,0,:]
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            if self.moving_mean is None:
                self.moving_mean = mean * (1 - self.alpha)
                self.moving_variance = var * (1 - self.alpha)
            else:
            ...
            self.output = np.divide(input - mean, np.sqrt(var + self.EPS))
        else:
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        mean = np.mean(input, axis=0)
        var = np.var(input, axis=0)
        out = np.divide(input - mean, np.sqrt(var + self.EPS))
        self.gradInput = (gradOutput - np.mean(gradOutput, axis=0) - np.mean(gradOutput*out, axis = 0)*out)/np.sqrt(var + self.EPS)
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training ==True:
            ...
            self.mask = np.ones_like(input[0])
            self.output = input
        return  self.output
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = input * (input > 0) + input * (input <= 0) * self.slope
        return  self.output
    
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        temp = (input > 0) + self.slope*(input <= 0)
        self.gradInput = gradOutput * temp
        return self.gradInput
...
class ELU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput * np.where(input > 0, np.ones_like(input), self.output + self.alpha)
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = gradOutput * np.divide(1, 1 + np.exp(-input))
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateGradInput(self, input, target):
        ...
        # Your code goes here. ################################################
        local_derivative = -1 / (target.shape[0] * input_clamp)
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target): 
        # Your code goes here. ################################################
        self.output = (-1/target.shape[0])*np.sum(target * input)
        return self.output

    def updateGradInput(self, input, target):          
        # Your code goes here. ################################################
        self.gradInput = -1/(target.shape[0])*target
        return self.gradInput
...
# 
# Current step learning rate: $$\text{lr}_t = \text{learning_rate} * \dfrac{\sqrt{1-\beta_2^t}}{1-\beta_1^t}$$
...
def adam_optimizer(variables, gradients, config, state):  
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients): 
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            np.add(current_var, -lr_t*var_first_moment/(np.sqrt(var_second_moment) + config['epsilon']), out = current_var)
