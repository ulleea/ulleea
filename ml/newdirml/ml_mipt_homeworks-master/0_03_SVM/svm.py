...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp(-torch.cdist(x_1, x_2,2) ** 2 / (2 * sigma ** 2))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        self.betas = torch.full(
            (n_obj, 1), fill_value=0.001, dtype=X.dtype, requires_grad=True)
        # I've also add bias to the model
