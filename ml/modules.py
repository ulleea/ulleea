...
class Sequential(Module):
    ...
    def updateOutput(self, input):
        ...
        for module in self.modules:
            ...
            last_output = module.forward(self.inputs[-1])
    ...
    def backward(self, input, gradOutput):
        ...
        for i, module in enumerate(self.modules[::-1]):
            gradOutput = module.backward(y_i[i], gradOutput)
...
class Linear(Module):
    ...
    def __init__(self, n_in, n_out):
        # чтобы вызвать конструктор от того класса, от которого мы отнаследовались
        super(Linear, self).__init__()

        # This is a nice initialization
        stdv = 1. / np.sqrt(n_in)
        # умножаем x*W^T, поэтому таки размеры
    ...
    def accGradParameters(self, input, gradOutput):
        # суммируем по нулевой размерности: batch_size
...
# import numpy as np
#
# batch_size, n_feats1, n_feats2 = 16, 5, 3
#
# gradOutput = np.random.randn(batch_size, n_feats2)
#
# gradOutput.shape
#
# W = np.random.randn(n_feats2, n_feats1)
#
# W.shape
#
# _input = np.random.randn(batch_size, n_feats1)
#
# a = _input[:, None, :] * gradOutput[:, :, None]
#
...
# a[:, None, :] + b
#
# a.sum(axis=0).T.shape
#
# np.arange(10)
#
# import torch
#
# a = nn.functional.softmax(torch.from_numpy(np.arange(10, dtype=float)), dim=0)
#
# b = nn.functional.softmax(torch.from_numpy(np.arange(10, dtype=float)) - 10., dim=0)
#
# torch.allclose(a, b)
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        # при работе с SoftMax принято вычитать максимальный элемент, т.к.
        # это ни на что не повлияет, но вычисления должны пройти без ошибок переполнения
        self.output = np.subtract(input, input.max(axis=1, keepdims=True))
        self.output = np.exp(self.output)
        self.output /= np.sum(self.output, axis=1).reshape(-1, 1)

        return self.output

    def updateGradInput(self, input, gradOutput):
        output = np.exp(np.subtract(input, input.max(axis=1, keepdims=True)))
        output /= np.sum(output, axis=1).reshape(-1, 1)
        # непонятно, почему работает
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.add(self.output, -np.log(np.sum(np.exp(self.output), axis=1, keepdims=True)))
        return self.output
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # f(x)= 0,01x при x<0 и x при x>= 0
        self.output = np.where(input < 0, np.multiply(self.slope, input), input)
        return self.output

    def updateGradInput(self, input, gradOutput):
        derivative = np.where(input < 0, self.slope, 1)
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        self.output = np.where(input < 0, np.multiply(self.alpha, (np.exp(input) - 1)), input)
        return self.output

    def updateGradInput(self, input, gradOutput):
        grad = np.where(input < 0, np.add(self.output, self.alpha), 1)
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        self.gradInput = np.multiply(gradOutput, 1 / (np.add(1, np.exp(-input))))
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        self.output = -np.multiply(target, np.log(input_clamp))
