...
def main(n):
    v = (0, max(round(n/3), 1), max(2*round(n/3), 2))
    s = area(n, v[1]-v[0], v[2]-v[1])
    
    if subtriangle(1, v[1]-1) and subtriangle(v[1]+1, v[2]-1) and subtriangle(v[2]+1, n-1):
...
def subtriangle(v0, v2):
    ...
    if subtriangle(v0+1, v1-1) and subtriangle(v1+1, v2-1):
        return True
